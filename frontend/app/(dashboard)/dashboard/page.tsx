...
type JiraTask = { key: string; summary: string; status?: string | null; assignee?: string | null; priority?: string | null; url?: string | null; metadata?: Record<string, unknown> };
type Message = { id: string; sender: 'user' | 'agent'; content: string; citations?: Citation[]; tasks?: unknown[]; retrievalStatus?: string };
type Result = { routing: { targetAgent: string }; content: string; citations: Citation[]; tasks: unknown[]; requiresConfirmation?: boolean; proposal?: JiraProposal };

function normalizeProposal(value: Record<string, unknown>): JiraProposal & { requiresConfirmation?: boolean } {
  return { action: value.action === 'UPDATE' || value.operation === 'update' ? 'UPDATE' : 'CREATE', issueKey: String(value.issueKey ?? value.issue_key ?? '') || undefined, summary: String(value.summary ?? '') || undefined, projectKey: String(value.projectKey ?? value.project_key ?? '') || undefined, status: String(value.status ?? '') || undefined, requiresConfirmation: Boolean(value.requiresConfirmation) };
}

function JiraTaskCard({ task }: { task: JiraTask }) {
  const fields = (task.metadata?.fields || {}) as Record<string, unknown>;
  const project = (task.metadata?.project_key || (fields.project as Record<string, unknown> | undefined)?.key) as string | undefined;
  const issueType = (task.metadata?.issue_type || (fields.issuetype as Record<string, unknown> | undefined)?.name) as string | undefined;
  const assignee = task.assignee || ((fields.assignee as Record<string, unknown> | undefined)?.displayName as string | undefined);
  const dueDate = fields.duedate as string | undefined;
  return <div className="mt-4 overflow-hidden rounded-xl border border-slate-700 bg-gradient-to-br from-slate-900 to-slate-950 shadow-lg"><div className="border-b border-slate-800 px-4 py-3"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2"><span className="rounded-md bg-red-500/15 px-2 py-1 font-mono text-xs font-bold text-red-300">{task.key}</span>{project && <span className="text-[11px] text-slate-500">{project}</span>}</div><h3 className="mt-2 text-sm font-semibold leading-5 text-white">{task.summary}</h3></div>{task.url && <a href={task.url} target="_blank" rel="noreferrer" className="shrink-0 rounded-lg bg-red-600 px-3 py-2 text-xs font-semibold text-white hover:bg-red-500">Open in Jira ↗</a>}</div><div className="mt-3 flex flex-wrap gap-2"><span className="rounded-full border border-slate-700 bg-slate-800 px-2.5 py-1 text-[11px] text-slate-300">{task.status || 'Open'}</span>{task.priority && <span className="rounded-full border border-amber-500/25 bg-amber-500/10 px-2.5 py-1 text-[11px] text-amber-200">Priority: {task.priority}</span>}{issueType && <span className="rounded-full border border-indigo-500/25 bg-indigo-500/10 px-2.5 py-1 text-[11px] text-indigo-200">{issueType}</span>}</div></div><details className="group"><summary className="cursor-pointer list-none px-4 py-3 text-xs font-medium text-slate-400 hover:text-slate-200"><span className="group-open:hidden">Show ticket details +</span><span className="hidden group-open:inline">Hide ticket details −</span></summary><div className="grid gap-3 border-t border-slate-800 px-4 py-4 text-xs sm:grid-cols-2"><div><p className="text-slate-500">Assignee</p><p className="mt-1 text-slate-200">{assignee || 'Unassigned'}</p></div><div><p className="text-slate-500">Due date</p><p className="mt-1 text-slate-200">{dueDate || 'No due date'}</p></div><div><p className="text-slate-500">Project</p><p className="mt-1 text-slate-200">{project || '—'}</p></div><div><p className="text-slate-500">Issue type</p><p className="mt-1 text-slate-200">{issueType || '—'}</p></div></div></details></div>;
}
...
export default function DashboardPage() {
  ...
  async function sendMessage(event?: FormEvent) {
    ...
    try {
      ...
      const response = await fetch('/api/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt: text, workspaceId: workspace?.id, context, stream: true }) });
      if (!response.ok || !response.body) {
        const failure = await response.json().catch(() => ({})) as { error?: string; detail?: string };
        throw new Error(failure.error || failure.detail || `Chat request failed (HTTP ${response.status})`);
      }
      ...
      const finalResult = { routing: result.routing || { targetAgent: 'knowledge' }, content: answer, citations: result.citations || [], tasks: result.requiresConfirmation && result.proposal ? [{ ...result.proposal, requiresConfirmation: true }] : (result.tasks || []) };
  ...
  async function confirmJiraAction(message: Message, proposal: JiraProposal, approved: boolean) {
    ...
    setBusy(true);
    try {
      proposal = normalizeProposal(proposal as unknown as Record<string, unknown>);
      const response = await fetch('/api/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt: proposal.action === 'CREATE' ? `Create Jira task ${proposal.summary || ''} in ${proposal.projectKey || ''}` : `Update ${proposal.issueKey || ''} to ${proposal.status || ''}`, workspaceId: workspace?.id, context: messages.slice(-8).map((item) => `${item.sender}: ${item.content}`).join('\n'), confirmation: { approved, proposal } }) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || 'Jira confirmation failed');
      setMessages((current) => current.map((item) => item.id === message.id ? { ...item, content: approved ? body.content : 'Jira action cancelled. No changes were made.', tasks: approved ? (body.tasks || []) : [] } : item));
    } catch (error) {
